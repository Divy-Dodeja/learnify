...
const transporter = nodemailer.createTransport({
  ...
  secure: false,
  auth: {
    user: 'postmaster@tools.surf',
    pass: '86293620202ece96c18648659899500e-71b35d7e-4a5a4f88',
  },
});

const sendEmail = async (to, subject, html, from = 'support@tools.surf') => {
...
const sendResetEmail = async (email, token) => {
  ...
  <body style="-ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%;
   ...
   padding-right:48px; padding-bottom:24px; padding-left:48px;" valign="middle">
                                          <a class="mcnButton " href="https://auth.panels.guru/auth/change-password?token=${token}" style="mso-line-height-rule: exactly;
  ...
  await sendEmail(email, 'Forget password', template);
};

const sendWelcomeEmail = async (username, email) => {
  const template = `
    <center>
    <h1 style="color:Cyan">Learnify</h1>
    <p style="font-size:20px">Welcome to world's largest Online learning platform</p>
    <p>Thank you for Signing up in Learnify</p>
    <div>
    ...
    display:block;
    position:relative;
    font-size:20px;" target="_BLANK">Start Exploring Now!</a>
    </div>
    </center>
    `;
  await sendEmail(email, 'Welcome to Learnify', template);
};
...
const sendConfirmationEmail = async (username, email, token) => {
  const template = `
      <center>
      <h1 style="color:Cyan">Learnify</h1>
      <p> ${email} Use below link to Confirm Your Account</p>
      <div>
      <a href="https://auth.panels.guru/auth/confirm?token=${token}" style="color:white;
      outline:none;
      text-decoration:none;
      background-color:cyan;
      padding:20px 30px;
      ...
      display:block;
      position:relative;
      font-size:20px;" target="_BLANK">Confirm Your Account Now Now!</a>
      </div>
      </center>
      `;
